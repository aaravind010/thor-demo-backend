...
inputs = {
  # --- network ---
  # qa now gets its own VPC instead of borrowing dev's — CIDR range shifted to 10.1.0.0/16 so it can't collide with dev/prod's 10.0.0.0/16 if the VPCs are ever peered.
