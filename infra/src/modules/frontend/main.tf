...
resource "aws_cloudfront_distribution" "thor-fe-cdn" {
  ...
  # Falls back to the default *.cloudfront.net certificate when domain_name isn't set — cloudfront_default_certificate and the acm_* fields are mutually exclusive as far as CloudFront's API is concerned, so exactly one side of this is non-null at a time, never both.
  viewer_certificate {
...
# Only created once domain_name is set — CloudFront's own hosted_zone_id (Z2FDTNDATAQYW2) is a fixed, well-known constant for every distribution globally, not something looked up per-region.
