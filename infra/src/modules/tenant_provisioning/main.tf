...
locals {
  ...
  # One published artifact, six functions — each a distinct handler in the same assembly.
  functions = {
  ...
  db_function_keys = ["seed-tenant-metadata", "create-tenant-database", "finalize-routing"]
  db_functions     = toset(local.db_function_keys)

  # All six share one composition root, which reads every var at cold start — so every Lambda
  ...
  # gets the DDL role; seed/finalize get the metadata-writer role.
  db_function_dbuser = {
  ...
  asl_path = "${var.source_dir}/../statemachine/tenant-provisioning.asl.json"
}
...
data "archive_file" "provisioning" {
  ...
  output_path = "${var.source_dir}/../tenant-provisioning-build.zip"
}
