locals {
  ...
  # Every ingestion compute target (both zip Lambdas, the per-step Lambdas, the ECS tasks) connects
  # through the RDS Proxy, so rds-db:connect is scoped by proxy resource ID — a cluster-scoped ARN
  # authorizes nothing on that leg. Two dbusers: the Master-DB read role and the per-tenant roles
  # (tenant_* covers _rw and _ro). Same shape as modules/ecs/iam.tf.
  rds_db_arn_prefix = "arn:aws:rds-db:${var.aws_region}:${var.account_id}:dbuser:${var.rds_proxy_resource_id}"
  rds_db_connect_resources = [
  ...
  # can't drift. There is no password auth path: both compute targets mint an RDS IAM token for
  # THOR_MASTERDB_USER in THOR_MASTERDB_REGION, so the map is now identical for both.
  # THOR_AWS_REGION is explicit because Fargate, unlike Lambda, never sets AWS_REGION.
