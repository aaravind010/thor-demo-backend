...
# Identical to the ECS task roles: the step code is the same image either way, and with RDS IAM
# auth (rds-db:connect, granted in the sourced task document) neither target needs anything the
# other doesn't. Kept as their own documents so a Lambda-only grant has somewhere to go.
...
resource "aws_lambda_function" "ingestion_step" {
  ...
  # Everything else, DB auth included, is identical to the ECS branch — local.workflow_environment
  # carries the RDS IAM user/region both targets mint a token with.
  environment {
