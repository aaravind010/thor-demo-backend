...
# CreateManifest itself starts the Step Functions execution, and resolves the Master DB credentials
# from the Aurora secret (THOR_MASTERDB_SECRET_ARN). One policy for everything beyond basic
# execution — add statements here as needed, not a new policy resource.
...
resource "aws_lambda_function" "manifest_lambda_function" {
  ...
  # No NAT on these subnets: Secrets Manager and Step Functions are reached through modules/network's
  # secretsmanager and states interface endpoints.
  vpc_config {
...
data "archive_file" "driver_lambda_archive" {
  ...
  output_path = "${var.ingestion_driver_source_dir}/../ingestion-driver-build.zip"
}
...
# other consumer, may resolve tenant secrets. One policy — add statements here, not a new resource.
data "aws_iam_policy_document" "driver_lambda_role_permissions_document" {
  ...
  statement {
    actions   = ["secretsmanager:GetSecretValue"]
    resources = [var.aurora_secret_arn, "arn:aws:secretsmanager:${var.aws_region}:${var.account_id}:secret:thor-${var.environment}-*"]
  }
}
