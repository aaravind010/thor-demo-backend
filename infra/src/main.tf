...
locals {
  # Flattens hosted_zones nested certificates for the flat route53/acm module inputs.

  # Zones for modules/route53, keyed by zone_name (not hosted_zones own logical keys).
  route53_zones = {
  ...
  # All certificates flattened into one map, keyed by "<zone_key>/<cert_key>" to avoid name collisions; zone_name rides along for the zone_id lookup below.
...
locals {
  # null until enable_route53 + frontend_certificate_key are both set, so module.frontend falls back to its own "no custom domain" defaults.
