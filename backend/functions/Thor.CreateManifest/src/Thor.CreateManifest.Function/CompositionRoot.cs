...
internal static class CompositionRoot
{
    public static IServiceProvider BuildServiceProvider()
    {
        ...
        var masterSecretArn = Environment.GetEnvironmentVariable("THOR_MASTERDB_SECRET_ARN")
            ?? throw new InvalidOperationException("THOR_MASTERDB_SECRET_ARN is not set.");
        var (masterUsername, masterPassword) = secretResolver.ResolveAsync(masterSecretArn).GetAwaiter().GetResult();

        var masterConnectionInfo = new MasterConnectionInfo(
            Host: Environment.GetEnvironmentVariable("THOR_MASTERDB_HOST")
                ?? throw new InvalidOperationException("THOR_MASTERDB_HOST is not set."),
            Database: Environment.GetEnvironmentVariable("THOR_MASTERDB_DATABASE")
                ?? throw new InvalidOperationException("THOR_MASTERDB_DATABASE is not set."),
