...
try
{
    ...
    // swap in a resolver that reads credentials from env vars instead (THOR_TENANTDB_*),
    // matching the MasterConnectionInfo/TaskApiSecurityOptions dev-fallback convention above.
    if (builder.Environment.IsDevelopment())
    {
        builder.Services.AddSingleton<ITenantSecretResolver, LocalTenantSecretResolver>();
    ...
    else
    {
        ...
        builder.Services.AddSingleton<ITenantSecretResolver, TenantSecretResolver>();
    ...
    builder.Services.AddSingleton(new MasterConnectionInfo(
        ...
        Password: RequiredEnvironment.GetVariable("THOR_MASTERDB_PASSWORD"),
