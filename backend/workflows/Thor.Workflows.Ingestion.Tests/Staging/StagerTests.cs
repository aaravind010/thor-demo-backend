...
public sealed class StagerTests : IAsyncLifetime
{
    ...
    private static ParsedAccount BuildAccount(Guid sourceId, string nativeId, string? email, string? domainName)
    {
        var account = new ParsedAccount(
            ...
            IsDeleted: false,
            IsDisabled: false,
            RawAttributes: new AccountRawAttributes(
                Dn: "cn=tuser,dc=test",
                ObjectClass: ["user"],
                Department: null,
                GivenName: null,
    ...
    private static ParsedGroup BuildGroup(Guid sourceId, string nativeId)
    {
        var group = new ParsedGroup(
            ...
            IsLargeGroup: false,
            IsDeleted: false,
            RawAttributes: new GroupRawAttributes(
                Dn: "cn=tgrp,dc=test",
    ...
    public async Task StageAsync_PreservesContentHashAndRawAttributesJson()
    {
        ...
        var rawAttributes = System.Text.Json.JsonSerializer.Deserialize<AccountRawAttributes>(staged.RawAttributes);
        var originalRawAttributes = (AccountRawAttributes)account.RawAttributes;
        Assert.NotNull(rawAttributes);
        Assert.Equal(originalRawAttributes.Dn, rawAttributes!.Dn);
