...
public sealed class CyberArkNormalizer(ILogger<CyberArkNormalizer> logger, IAttributeMapProvider mapProvider) : IConnectorNormalizer
{
    public IngestBatch Normalize(byte[] rawExportBytes, Guid sourceId)
    {
        ...
        var (assets, safeNameToNumber) = BuildSafes(root, sourceId, safeMap);
        ...
        var (memberAccounts, memberGroups, droppedMembers) = BuildMembers(root, sourceId, memberAccountMap, memberGroupMap);
    ...
    private static (List<ParsedAsset> Assets, Dictionary<string, string> SafeNameToNumber) BuildSafes(JsonObject root, Guid sourceId, AttributeMap map)
    {
        ...
        {
            return (assets, safeNameToNumber);
        }

        foreach (var node in safes)
        {
            if (node is not JsonObject raw || raw["SafeNumber"] is not JsonValue safeNumberNode)
            {
                continue;
            }

            var safeNumber = safeNumberNode.GetValue<int>().ToString();
    ...
    /// </summary>
    private static (List<ParsedAccount> Accounts, List<ParsedGroup> Groups, int Dropped) BuildMembers(
        JsonObject root, Guid sourceId, AttributeMap memberAccountMap, AttributeMap memberGroupMap)
    {
        ...
        {
            return (accounts, groups, dropped);
        }
        ...
        foreach (var ((memberType, memberName), rows) in byIdentity)
        {
            if (memberType == "Group")
            {
                groups.Add(BuildMemberGroup(memberName, rows, sourceId, memberGroupMap));
            }
            else
            {
                accounts.Add(BuildMemberAccount(memberName, rows, sourceId, memberAccountMap));
            }
        }
    ...
    private static ParsedAccount BuildMemberAccount(string memberName, IReadOnlyList<JsonObject> rows, Guid sourceId, AttributeMap map)
    {
        var edgeRefs = BuildHasAccessEdgeRefs(rows);
    ...
    private static ParsedGroup BuildMemberGroup(string memberName, IReadOnlyList<JsonObject> rows, Guid sourceId, AttributeMap map)
    {
        var edgeRefs = BuildHasAccessEdgeRefs(rows);
        var extra = UnmappedAttributeCollector.Collect(rows[0], map);
        var rawAttributes = new RawAttributesWithEdges(AliasKeys: [memberName.ToLowerInvariant()], EdgeRefs: edgeRefs, Extra: extra);
    ...
    private static List<EdgeRef> BuildHasAccessEdgeRefs(IReadOnlyList<JsonObject> memberRows)
    {
        var edgeRefs = new List<EdgeRef>();
        foreach (var row in memberRows)
        {
            if (row["SafeNumber"] is not JsonValue safeNumberNode)
            {
                continue;
            }
            var safeNumber = safeNumberNode.GetValue<int>().ToString();
