...
public sealed class EdgeResolver(OneSidedRefIndex oneSidedRefIndex, ILogger<EdgeResolver> logger)
{
    ...
    public async Task<EdgeResolutionResult> ResolveFullAsync(TenantDbContext db, Guid scanId, Guid scanManifestId, IReadOnlySet<string> twoSidedRelTypes, CancellationToken cancellationToken = default)
    {
        await using var transaction = await db.Database.BeginTransactionAsync(cancellationToken);

        await SeedAllOwnersAsync(db, cancellationToken);
        await oneSidedRefIndex.RefreshForSeedAsync(db, twoSidedRelTypes, cancellationToken);

        await InsertOwnerScopedStagingRowsAsync(db, scanManifestId, cancellationToken);
        var (inserted, updated) = await PromoteStagedEdgesAsync(db, scanId, scanManifestId, cancellationToken);
        var removed = await ApplyOwnerScopedRemovalsAsync(db, scanId, scanManifestId, cancellationToken);
        await CleanupAsync(db, scanManifestId, cancellationToken);
    ...
    /// </summary>
    public async Task<EdgeResolutionResult> ResolveIncrementalAsync(TenantDbContext db, Guid scanId, Guid scanManifestId, IReadOnlySet<string> twoSidedRelTypes, CancellationToken cancellationToken = default)
    {
        await using var transaction = await db.Database.BeginTransactionAsync(cancellationToken);

        await SeedChangedOwnersAsync(db, scanId, scanManifestId, cancellationToken);
        await oneSidedRefIndex.RefreshForSeedAsync(db, twoSidedRelTypes, cancellationToken);

        await InsertDeltaStagingRowsAsync(db, scanManifestId, scanManifestId, cancellationToken);
        var (inserted, updated) = await PromoteStagedEdgesAsync(db, scanId, scanManifestId, cancellationToken);
        var removed = await ApplyDeltaRemovalsAsync(db, scanId, scanManifestId, scanManifestId, cancellationToken);
        await CleanupAsync(db, scanManifestId, cancellationToken);
    ...
    private static async Task SeedAllOwnersAsync(TenantDbContext db, CancellationToken cancellationToken)
    {
        await SqlExec.ExecAsync(db, "DROP TABLE IF EXISTS seed_owner", cancellationToken);
        await SqlExec.ExecAsync(db, "CREATE TEMP TABLE seed_owner (id uuid PRIMARY KEY, owner_type text NOT NULL)", cancellationToken);

        var unionSql = string.Join("\nUNION ALL\n", OwnerRefsSql.OwnerTables.Select(t => $"SELECT id, '{t.EntityType}' FROM {t.Table}"));
        await SqlExec.ExecAsync(db, $"INSERT INTO seed_owner (id, owner_type)\n{unionSql}", cancellationToken);
    }
    ...
    private static async Task SeedChangedOwnersAsync(TenantDbContext db, Guid scanId, Guid scanManifestId, CancellationToken cancellationToken)
    {
        await SqlExec.ExecAsync(db, "DROP TABLE IF EXISTS seed_owner", cancellationToken);
        await SqlExec.ExecAsync(db, "CREATE TEMP TABLE seed_owner (id uuid PRIMARY KEY, owner_type text NOT NULL)", cancellationToken);
    ...
    private static async Task InsertOwnerScopedStagingRowsAsync(TenantDbContext db, Guid gateManifestId, CancellationToken cancellationToken)
    {
        var sql = $"""
            ...
            FROM deduped
            """;
        await SqlExec.ExecAsync(db, sql, cancellationToken, new NpgsqlParameter("gate_manifest_id", gateManifestId));
    }
    ...
    private static async Task InsertDeltaStagingRowsAsync(TenantDbContext db, Guid scanManifestId, Guid gateManifestId, CancellationToken cancellationToken)
    {
        var sql = $"""
            INSERT INTO tenant.staging_edge (scan_manifest_id, batch_seq, from_id, from_type, to_id, to_type, rel_type, props, content_hash, received_at)
            WITH owner_refs AS (
    ...
    private static async Task<(int Inserted, int Updated)> PromoteStagedEdgesAsync(TenantDbContext db, Guid scanId, Guid gateManifestId, CancellationToken cancellationToken)
    {
    ...
    private static async Task<int> ApplyOwnerScopedRemovalsAsync(TenantDbContext db, Guid scanId, Guid gateManifestId, CancellationToken cancellationToken)
    {
        await SqlExec.ExecAsync(db, "DROP TABLE IF EXISTS cand_removal", cancellationToken);
            ...
            WHERE e.is_deleted = FALSE
              AND NOT EXISTS (
                  ...
                  WHERE s.scan_manifest_id = @gate_manifest_id
                    AND s.from_id = e.from_id AND s.to_id = e.to_id AND s.rel_type = e.rel_type)
            """;
        await SqlExec.ExecAsync(db, candidateSql, cancellationToken, new NpgsqlParameter("gate_manifest_id", gateManifestId));

        return await VerifyAndApplyRemovalsAsync(db, scanId, gateManifestId, cancellationToken);
    }

    /// <summary>B7 removal candidates: only the specific edge tuples this manifest's removed refs used to produce.</summary>
    private static async Task<int> ApplyDeltaRemovalsAsync(TenantDbContext db, Guid scanId, Guid scanManifestId, Guid gateManifestId, CancellationToken cancellationToken)
    {
        await SqlExec.ExecAsync(db, "DROP TABLE IF EXISTS cand_removal", cancellationToken);
        var candidateSql = $"""
            CREATE TEMP TABLE cand_removal AS
    ...
    /// </summary>
    private static async Task<int> VerifyAndApplyRemovalsAsync(TenantDbContext db, Guid scanId, Guid gateManifestId, CancellationToken cancellationToken)
    {
        var candidateCount = await SqlExec.ScalarLongAsync(db, "SELECT count(*) FROM cand_removal", cancellationToken);
        if (candidateCount == 0)
        {
            return 0;
        }

        await SqlExec.ExecAsync(db, "DROP TABLE IF EXISTS cand_ep", cancellationToken);
        await SqlExec.ExecAsync(db, "CREATE TEMP TABLE cand_ep AS SELECT from_id AS id FROM cand_removal UNION SELECT to_id FROM cand_removal", cancellationToken);

        await SqlExec.ExecAsync(db, "DROP TABLE IF EXISTS cand_produced", cancellationToken);
        var producedSql = $"""
            ...
            """;
        await SqlExec.ExecAsync(db, producedSql, cancellationToken);
    ...
    private static async Task CleanupAsync(TenantDbContext db, Guid gateManifestId, CancellationToken cancellationToken)
    {
        await SqlExec.ExecAsync(db, "DELETE FROM tenant.staging_edge WHERE scan_manifest_id = @gate_manifest_id", cancellationToken, new NpgsqlParameter("gate_manifest_id", gateManifestId));
        await SqlExec.ExecAsync(db, "DROP TABLE IF EXISTS seed_owner", cancellationToken);
        await SqlExec.ExecAsync(db, "DROP TABLE IF EXISTS cand_removal", cancellationToken);
        await SqlExec.ExecAsync(db, "DROP TABLE IF EXISTS cand_ep", cancellationToken);
        await SqlExec.ExecAsync(db, "DROP TABLE IF EXISTS cand_produced", cancellationToken);
