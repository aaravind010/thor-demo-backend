...
public sealed class OneSidedRefIndex
{
    ...
    public async Task RefreshForSeedAsync(TenantDbContext db, IReadOnlySet<string> twoSidedRelTypes)
    {
        await SqlExec.ExecAsync(db, "DELETE FROM tenant.one_sided_ref WHERE owner_id IN (SELECT id FROM seed_owner)");
        ...
        await SqlExec.ExecAsync(db, sql, twoSided);
    }
