...
///
/// <c>RawAttributes</c> is deliberately untyped (<see cref="object"/>): each connector defines
/// its own raw-attributes shape (e.g. AD's <see cref="AccountRawAttributes"/>) — the only
/// contract every shape must satisfy is "JSON-serializable", enforced by <see cref="Staging.Stager"/>
