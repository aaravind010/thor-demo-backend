...
staging row's `raw_attributes` JSON column, either via `UnmappedAttributeCollector`
(CyberArk, Windows) or AD's own hand-curated `AccountRawAttributes`/`GroupRawAttributes`
shape (AD predates the generic collector). `raw_attributes` also always carries the
entity's `AliasKeys`/`EdgeRefs` deferred-reference contract — for Windows and every
CyberArk entity kind this is the shared `RawAttributesWithEdges` record; AD keeps its own
shape with the same two fields inlined.
...
| `raw_attributes` | `dn`, `objectClass`, `department`, `givenName`, `sn`, `employeeID`, `memberOf`, `manager` | Hand-curated `AccountRawAttributes` (not the generic collector): carries the above fields plus `aliasKeys` (`[dn.lower()]`) and `edgeRefs` (`memberOf` → `MEMBER_OF` out to `grp`; `manager` → `REPORTS_TO` out to `account`) |
...
| `raw_attributes` | `dn`, `member`, `memberOf`, `managedBy` | Hand-curated `GroupRawAttributes`: `aliasKeys` (`[dn.lower()]`) and `edgeRefs` (`member` → `MEMBER_OF` in; `memberOf` → `MEMBER_OF` out; `managedBy` → `MANAGED_BY` out) |
...
- **CyberArk's `HAS_ACCESS`/`STORED_IN` edges carry data beyond a plain reference** — the
  ...
  Entitlement records.
- **AD is the only connector still on its own hand-curated raw-attributes shape**
  (`AccountRawAttributes`/`GroupRawAttributes`) rather than the shared
  `RawAttributesWithEdges` record CyberArk and Windows use — same `aliasKeys`/`edgeRefs`
  contract, just not consolidated into one type.
